...
function emitSnapshot(ws: Bun.ServerWebSocket<ConnectionState>) {
  ...
  send(ws, {
    type: "session_state",
    snapshot: snapshotSession(data.session, data.status, data.model),
  });
}
...
async function handleChat(ws: Bun.ServerWebSocket<ConnectionState>, event: ClientChatEvent) {
  ...
  try {
    for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
      ...
      send(ws, {
        type: "session_state",
        snapshot: snapshotSession(session, ws.data.status, model),
      });
    ...
    send(ws, {
      type: "error",
      message: "OpenAI Antwort fehlgeschlagen.",
      ...
      if (event.type === "init") {
        if (!OPENAI_API_KEY) {
          ...
          send(ws, {
            ...
            detail: "Lege den Token in agentv2/.env ab.",
          });
          return;
        }
        ...
        ws.data = {
          model: event.model.trim() || "gpt-4o-mini",
          session,
          status: "ready",
        };
        send(ws, {
          type: "ready",
          snapshot: snapshotSession(session, "ready", ws.data.model),
        });
        return;
      }
