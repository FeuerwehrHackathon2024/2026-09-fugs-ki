...
log = logging.getLogger("rescue_tablet")

class RescueTabletClient:
    """HTTP-Client für die RescueTablet REST API (statischer Bearer-Token)."""
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._base_url = f"https://{self.settings.rescue_tablet_host}/api/v1"
        self._sec_header_name = self.settings.rescue_tablet_security_header_name
        self._sec_header_value = self.settings.rescue_tablet_security_header_value
        self._bearer_token = self.settings.rescue_tablet_bearer_token
        self._verify_ssl = self.settings.rescue_tablet_verify_ssl
        self._user_agent = self.settings.rescue_tablet_user_agent
